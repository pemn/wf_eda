...
def mr_boolean(meshes, operation, cell_size = 10.0):
  global mr
  if mr is None:
    import meshlib.mrmeshpy as mr
  if not operation:
  ...
  for mesh1 in map(vtk_pv2mr, meshes):
    if mesh0 is None:
      mesh0 = mesh1
      continue
    if operation in ('voxelIntersect', 'voxelSubtract', 'voxelUnite'):
...
def mr_block_mine(meshes, centroids, lengths = None):
  if not isinstance(meshes, (list, tuple)):
    meshes = [meshes]
  vf = [mr.Vector3f(x, y, z) for x,y,z in centroids]
  if lengths is None:
    lengths = np.zeros(centroids.shape[0])
  elif isinstance(lengths, (float, int)):
    lengths = np.full(centroids.shape[0], lengths)

  mine = np.full(centroids.shape[0], np.nan)
  for mesh in meshes:
    for ri, rd in enumerate(vf):
      sd = mr.findSignedDistance(rd, mesh)
      if sd.dist <= lengths[ri]:
        p = 1.0
        if lengths[ri] > 0:
          if sd.dist > 0:
            p =  ( lengths[ri] - sd.dist ) / lengths[ri]
          else:
            p = np.minimum(1.0, np.abs(sd.dist) / lengths[ri])
...
def vtk_block_mine(meshes, grid):
  if not isinstance(meshes, (list, tuple)):
    meshes = [meshes]
  mine = np.full(grid.n_cells, np.nan)
  lengths = np.full(grid.n_cells, np.mean(grid.spacing))
  for mesh in meshes:
    g = grid.compute_implicit_distance(mesh)
    d = g.ptc().get_array('implicit_distance')
    p = np.where(np.greater(d, 0), np.where(np.greater(d, lengths), np.nan, np.divide(np.subtract(lengths, d), lengths)), np.minimum(1.0, np.divide(np.abs(d), lengths)))

    mine = np.where(np.isnan(mine), p, np.multiply(mine, np.where(np.isnan(p), 1.0, p)))
  return mine
...
def vtk_plot_meshes(meshes, point_labels=False, cmap = None, scalars = None, show = True):
  # plt.cm.terrain
  # plt.cm.plasma
  # plt.cm.gray
  # plt.cm.spectral
    ...
    if mesh is not None and mesh.n_points:
      ...
      if hasattr(mesh, 'textures') and len(mesh.textures):
        ...
        if mesh_scalars is not None and mesh.get_array(mesh_scalars).dtype.hasobject:
            mesh_scalars = None
        p.add_volume(mesh, opacity=1, scalars=mesh_scalars)
      elif mesh_scalars:
    ...
    if i < len(meshes) and meshes[i] is not None:
      ...
      for mesh in pieces:
        if hasattr(mesh, 'topology'):
          ...
          v = mn.toNumpyArray(mesh.points)
          if len(v):
            ax.flat[i].scatter(*v.T)
        elif mesh.shape[1] == 4:
          ax.flat[i].scatter(mesh[:,0], mesh[:,1], mesh[:,2], c=mesh[:,3])
        else:
          ax.flat[i].scatter(*mesh.T)
  ...
  if isinstance(mesh, list) or hasattr(mesh, 'n_blocks'):
    ...
    if mesh.GetDataObjectType() in [2,4,6]:
      ...
      if len(mesh.cell_data) == 0 and len(mesh.point_data) > 0 and mesh.n_points:
        mesh = mesh.ptc()
      arr_data = [pd.Series(mesh.get_array(name), name=name) for name in mesh.cell_data if name not in xyz]
    else:
      arr_data = []
      ...
      if mesh.n_cells and len(mesh.faces):
        ...
        arr_data = [pd.Series(mesh.get_array(name), name=name) for name in mesh.array_names if np.ndim(mesh.get_array(name)) == 1 and name not in xyz]
...
def vtk_mesh_info(mesh):
  ...
  if hasattr(mesh, 'n_blocks'):
    ...
    text += vtk_mesh_data(mesh)
  return text

def vtk_mesh_data(mesh):
  text = chr(10)
...
def vtk_array_ijk(self, array_name = None, cell = None, text_to_index = False):
  s = None
  ...
  if s.dtype.num >= 17 and text_to_index:
...
def vtk_cell_size(self):
  ...
  if hasattr(self, 'spacing'):
    return self.spacing
  b0 = np.reshape(self.bounds, (2,3), order='F')
...
def vtk_plot_grids(grids, variable = None, threshold = None):
  ...
  for i in range(len(grids)):
    ...
    v = np.ones(s.shape, dtype=np.bool_)
    if threshold is not None:
      v = np.greater(s, threshold)
    ...
    ax.flat[i].voxels(v, facecolors=cmap(s))
...
class vtk_Voxel(object):
  ...
  def from_bb_schema(cls, bb, schema, ndim = 3, border = 1):
    ...
    if bearing != 90.0:
      ...
      bb_r = np.reshape(mesh.bounds, (2,3), order='F')
  ...
  def from_mesh(cls, mesh, cell_size = 10, ndim = 3):
    bb = np.reshape(mesh.bounds, (2,3), order='F')
  ...
  def from_tif(cls, fp, cell_size=None, origin=None):
    if cell_size is None:
      cell_size = np.ones(3)
    return cls.factory(vtk_tif_to_grid(fp, cell_size))
  ...
  def __repr__(self):
    return super().__repr__() + vtk_mesh_data(self)
...
def vtk_path_to_texture(fp):
  ...
  return pv.Texture(np.flipud(img))
